...
#
#  Benchmarks
#    Absolute:  tm_N = 1 where n_sp_new <= n_sp_crit =
...
#     5  Policy indicators        tm_N (ND/WFD) and nsu50 (F2F proxy)
...
#     9  Assessment table         nd_wfd_assessment.csv with benchmarks
#    10  Run metadata             provenance record saved to
#                                 run_meta_script5.rds
#    11  Figures                  all Chapter 4 SRQ3 figures:
#                                 Fig_nuts2_ecdf.png   ECDF N-surplus
#                                 Fig_nuts2_ecdf_nd.png ECDF ND/WFD compliance
#                                 Fig_country_bars.png country bar chart
#                                 Fig_nd_vs_f2f.png    country scatter
#                                 Fig_nuts2_nsu_red.png NUTS-2 choropleth
#                                 Fig_nuts2_nd_wfd.png  NUTS-2 choropleth
#                                 Fig_benchmark_maps.png two-map divergence panel (report)
#                                 Fig_benchmark_ecdf.png two-ECDF panel (report)
...
na_tmN <- bn[is.na(tm_N), sum(aw, na.rm = TRUE)]
log('Area with undefined tm_N: ', round(na_tmN / 1000, 1),
    ' kha (', round(100 * na_tmN / area_out, 2), '% of total)')


# 5. Policy indicators per NCU-NUTS2 piece ---------------------------------
# Absolute:   tm_N = 1 where n_sp_new <= n_sp_crit (computed in dst_functions_fert.R)
...
aggregate_impacts <- function(dt, by_cols) {
  dt[, .(
    ...
    pct_area_nd_wfd = weighted.mean(tm_N,  aw, na.rm = TRUE) * 100,
...
nd_cols <- c('country', 'area_kha', 'nsu_red_kgha', 'nsu_red_pct',
             'pct_area_nd_wfd', 'pct_area_nsu50')
...
# 10. Run metadata ---------------------------------------------------------
run_meta <- list(
  script              = 'Script_5_nuts_2_N_assessment_23052026_clean.R',
  started_ts          = run_ts,
  ...
  benchmark_absolute  = 'ND/WFD groundwater-nitrate limit (n_sp_crit; 11.3 mg NO3-N/L; site-specific)',
